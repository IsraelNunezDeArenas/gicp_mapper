// ═══════════════════════════════════════════════════════════════════════════════
//  gicp_mapper_node.cpp  —  VGICP mapper con color y guardado PLY
//
//  QoS: RELIABLE / KEEP_LAST compatible con publishers Python.
//  Cola interna acotada (latest-wins) para desacoplar recepción y VGICP.
//
//  Parámetros relevantes:
//    vgicp_resolution      resolución del vóxel interno de VGICP   (def. 0.3 m)
//    source_max_points     puntos del source por frame              (def. 5000)
//    local_map_window      keyframes en la ventana del target       (def. 20)
//    save_path             ruta del fichero PLY de salida
//    register_policy       always | converged_only | best_fitness
// ═══════════════════════════════════════════════════════════════════════════════
...
public:
  ...
  , stop_worker_(false)
  {
    ...
    // Resolución del vóxel del target.
    // Valor más pequeño → más precisión, más lento.
    // Valor más grande  → más rápido, menos detalle geométrico.
    declare_parameter("vgicp_resolution",   0.3);
    // Puntos del source por frame (conteo fijo para latencia constante).
    declare_parameter("source_max_points",  5000);
    ...
    // Guardado PLY
    declare_parameter("save_path", "/tmp/gicp_map.ply");
    ...
    p.vgicp_resolution        = get_parameter("vgicp_resolution").as_double();
    p.source_max_points       = get_parameter("source_max_points").as_int();
    ...
    RCLCPP_INFO(get_logger(),
      "[gicp_mapper] OK — VGICP\n"
      "  topic            : %s\n"
      "  QoS              : RELIABLE / KEEP_LAST(%d)\n"
      "  proc_queue_size  : %zu\n"
      "  vgicp_resolution : %.2f m\n"
      "  source_max_pts   : %d\n"
      "  register_policy  : %s\n"
      ...
      get_parameter("save_path").as_string().c_str());
  }
...
private:
    ...
    const rclcpp::Time &)
  {
    ...
    if (max_cov_trace_ > 0.0) {
      double tr = pwc.covariance[0] + pwc.covariance[7] + pwc.covariance[14];
      if (tr > max_cov_trace_) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
          "Guess descartado: traza_cov=%.4f > max=%.4f", tr, max_cov_trace_);
        return std::nullopt;
      }
    }
  ...
  void publishMap()
  {
    ...
    RCLCPP_DEBUG(get_logger(),
      "Mapa local publicado: %zu pts  %d kf  [reg:%u rej:%u drop:%u]",
  ...
  // ── Guardar mapa (PLY binario con color XYZRGB) ───────────────────────────
  ...
  void doSave(std_srvs::srv::Trigger::Response::SharedPtr res)
  {
    ...
    // Construye el mapa global completo con color y lo guarda como PLY binario.
    // saveMapPLY() llama a buildGlobalMap() internamente.
    RCLCPP_INFO(get_logger(), "Generando mapa global (%d kf)…", mapper_->keyframeCount());

    if (mapper_->saveMapPLY(path)) {
      // Obtener número de puntos del mapa sin reconstruirlo de nuevo
      auto map = mapper_->buildGlobalMap();
      res->success = true;
      res->message =
        "Guardado PLY: " + path +
        "\n  formato  : PLY binario (XYZRGB, color original)" +
